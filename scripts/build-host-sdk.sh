#!/usr/bin/env bash
# Generate the complete, host-only SDK from a pinned Ubuntu Base.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PACKAGES_FILE="${HOST_SDK_PACKAGES_FILE:-$REPO_ROOT/config/host-sdk-packages.txt}"
LOCK_FILE="${HOST_SDK_LOCK_FILE:-$REPO_ROOT/config/host-sdk.lock}"
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/artifacts}"
CACHE_DIR="${HOST_SDK_DOWNLOAD_DIR:-$HOME/.cache/amphora-host-sdk}"
...
source "$LOCK_FILE"
: "${SDK_TAG:?missing SDK_TAG in $LOCK_FILE}"
...
mapfile -t packages < <(sed 's/#.*//; /^[[:space:]]*$/d' "$PACKAGES_FILE")
[ "${#packages[@]}" -gt 0 ] || {
...
if ! printf '%s  %s\n' "$UBUNTU_BASE_SHA256" "$base_archive" |
